...
class AcquirerPayson(models.Model):
    ...
    def payson_form_generate_values(self, partner_values, tx_values):
        ...
        payson_tx_values.update({
            'payson_testmode': self.environment
            
            "AgentId": self.payson_agent_id,
            "SellerEmail": '',
            "Description": '',
            "Cost": '',
            "ExtraCost": '',
        ...
        #payson_tx_values['MD5'] = self._payson_generate_checksum(payson_tx_values)
        return partner_values, payson_tx_values
...
class TxPayson(models.Model):
    ...
    def payson_init_payment(self):
        ...
        headers={
            ...
            'PAYSON-SECURITY-PASSWORD': self.payson_key, #Required. Your API Password (MD5-key).
        ...
        post={
            ...
            'returnUrl': ,   #Required. string (2048). URL to which the customer's browser is redirected after the payment is completed. Note: This includes both successful and unsuccessful payments.
            'cancelUrl': ,	#Required. string (2048). URL to which the customer is redirected if the payment is manually canceled by the user before it is completed.
            'ipnNotificationUrl': , #Recommended. string (2048). The URL for receiving an Instant Payment Notification about this payment. Note: This parameter has been made optional due to backward compatability. Read more about the use of it here »
            'memo': self.reference, #Required. string (128). Description of items the customer is purchasing.
            'senderEmail' self.partner_email: , #Required. string (128). Email address of the person sending money. This is the Payson account where the settled amount is transferred from.
            'senderFirstName': , #Required. string (128). First name of the buyer as entered during checkout.
            'senderLastName': , #Required. string (128). Last name of the buyer as entered during checkout.
            
            'localeCode': self.partner_country_id.code, #Optional. LocaleCode (SV/EN/FI/DK/NO). Locale of pages displayed by Payson during payment. Default: SE
            'currencyCode': self.currency_id.name, #Optional. CurrencyCode (SEK/EUR). The currency used for the payment. Default: SEK
            'feesPayer': self.payson_fees_payer, #Optional. FeesPayer (SENDER/PRIMARYRECEIVER). The payer of the Payson fees. Default: PRIMARYRECEIVER
            ...
            'guaranteeOffered': self.payson_guarantee, #Optional. GuaranteeOffered (OPTIONAL/REQUIRED/NO). Whether Payson Guarantee is offered or not. Default: OPTIONAL
            'showReceiptPage': self.payson_show_receipt, #Optional. bool. Whether to show the receipt page in Paysons checkout. Default: true
            
            #Receiver Details
            #The list of receivers. If you have more than one receiver you must specify exactly one as primary.
            'receiverList.receiver0.email': , #Required. string (128). Email address of the receiver.
            'receiverList.receiver0.amount': self.amount, #Required. decimal. The amount (including VAT) to transfer to this recipient. Note: If you have more than one receiver, the primary receiver's amount must still be the full amount of the payment.
        ...
        urllib2.Request('https://api.payson.se/1.0/Pay/', data=werkzeug.url_encode(post), headers=headers)

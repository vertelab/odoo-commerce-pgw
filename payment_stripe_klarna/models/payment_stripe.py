    ...
    def _add_available_payment_method_types(self, stripe_session_data, tx_values):
        ...
        all_payment_method_types = [
            PMT('klarna',
                ...
                ['eur', 'sek', 'nok', 'dkk', 'gdp'],
                # usd seem ok in doc but not in Odoo - Note: usd doesn't support 'Pay Now'
                'recurring'),
        ]
        ...
        pmt_currency_filtered = filter(lambda pmt: not pmt.currencies or currency in pmt.currencies,
                                       pmt_country_filtered)
        pmt_recurrence_filtered = filter(
            lambda pmt: tx_values.get('type') != 'form_save' or pmt.recurrence == 'recurring',
